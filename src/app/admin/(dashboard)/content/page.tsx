...
import type { AboutSettings, HeroSettings, InstagramSettings, WhyOrgnCard } from '@/lib/types';

export const metadata = { title: 'Content' };

export default async function AdminContentPage() {
  const supabase = await createClient();
  const [{ data: heroRow }, { data: whyRow }, { data: aboutRow }, { data: instagramRow }, { count: subscriberCount }] =
    await Promise.all([
      ...
      supabase.from('site_settings').select('value').eq('key', 'about').single(),
      supabase.from('site_settings').select('value').eq('key', 'instagram').single(),
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="flex flex-col gap-8">
        ...
        <section className="border-2 border-ink bg-offwhite p-6 shadow-orgn-sm">
          <h2 className="mb-4 font-display text-lg font-bold uppercase">About Section</h2>
          <AboutSettingsForm initial={aboutRow?.value as AboutSettings} />
        </section>
        ...
        <section className="border-2 border-ink bg-offwhite p-6 shadow-orgn-sm">
          <h2 className="mb-4 font-display text-lg font-bold uppercase">Follow the Drop — Instagram</h2>
          <InstagramSettingsForm initial={instagramRow?.value as InstagramSettings} />
        </section>
